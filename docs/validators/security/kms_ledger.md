...
- Create a `~/.tmkms/tmkms.toml` file with the following content (use an adequate `chain_id`)
  ...
  chain_id = "torque_9001-1"
  reconnect = true                                # true is the default
  secret_key = "~/.tmkms/secret_connection.key"

  [[providers.ledger]]
  chain_ids = ["torque_9001-1"]
  ```

- Edit `addr` to point to your `torqued` instance.
- Adjust `chain-id` to match your `.torqued/config/config.toml` settings.
...
The last step is to retrieve the validator key that you will use in `torqued`.
...
07:28:24 [INFO] [keyring:ledger:ledger] added validator key torquevalconspub1zcjduepqy53m39prgp9dz3nz96kaav3el5e0th8ltwcf8cpavqdvpxgr5slsd6wz6f
07:28:24 [INFO] KMS node ID: 1BC12314E2E1C29015B66017A397F170C6ECDE4A
```

The KMS may complain that it cannot connect to `torqued`. That is fine, we will fix it in the next section.
...
Let's assume that you have set up your validator account and called it `kmsval`. You can tell torqued the key that we've got in the previous section.

```bash
torqued gentx --name kmsval --pubkey <pub_key>
```

Now start `torqued`. You should see that the KMS connects and receives a signature request.
