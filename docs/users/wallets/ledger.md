...
Get started with your Ledger hardware wallet on Torque {synopsis}
...
If you want to connect to Torque mainnet and Torque testnet, you can use the Ethereum Ledger app on Ledger Live by setting the chain ID.
...
::: tip
Follow our [Metamask Guide](./metamask.md) to add the Torque Mainnet and Testnet to your Settings
:::
...
### Step-by Cosmos transaction using Torque.me
...
- Make sure that the Ethereum Ledger app is installed. The Cosmos Ledger app is not supported on the Torque chain at the moment (see [FAQ](#faq)).
...
Since Torque only support Ethereum keys and uses the same HD path as Ethereum, the Cosmos Ledger app doesn’t work to sign cosmos transactions.
